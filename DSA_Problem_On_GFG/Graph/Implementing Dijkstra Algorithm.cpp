/*
Q_Name:- Implementing Dijkstra Algorithm.
Graph algorithm
Description:- 
...
class Solution
{
    ...
    {
        // unordered_map<int, vector<pair<int, int>> > new_adj;
        
        // for(int i = 0; i < V; i++) {
        //     int u = i;
            
        //     for(int j = 0; j < adj[i].size(); j++) {
        //         new_adj[u].push_back({adj[i][j][0], adj[i][j][1]});
        //     }
        // }
        ...
        //         int curr_dis = v.second;
                
        //         if(curr_dis + d < ans[curr_node]) {
        //             ans[curr_node] = curr_dis + d;
        //             pq.push({curr_dis + d, curr_node});
        //         }
        ...
        //Using sets in cpp
        //steps 1:- build graph using unordered_map
        unordered_map<int, vector<pair<int, int>> > new_adj; //node-> (node, dis)
        ...
        while(!st.empty()) {
            ...
            for(auto &v : new_adj[node]) {
                ...
                if(curr_dis + d < ans[curr_node]) {
                    ...
                    if(ans[curr_node] != INT_MAX) {
                        //erasr that node and dis present in set
                        st.erase({ans[curr_node], curr_node});
                    }
                    
                    ans[curr_node] = curr_dis + d;
                    st.insert({curr_dis + d, curr_node});
                }
