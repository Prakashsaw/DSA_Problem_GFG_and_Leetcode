...
public:
    ...
    vector<int> solve_dfs(Node* root, int k) {
        vector<int> v(26, 0);
        if(root == NULL) return v;
        
        v[root->data]++;
        
        vector<int> l = solve_dfs(root->left, k);
        vector<int> r = solve_dfs(root->right, k);
        
        int curr_cnt = 0;
        for(int i = 0; i <= 25; i++) {
            if(l[i] != 0 || r[i] != 0 || v[i] != 0) curr_cnt++;
