/*
Q_Name:- frogs and Jumps
Platform:- gfg
Difficulty:- Easy.

Q_Description:- N frogs are positioned at one end of the pond. All frogs want to reach the other end of the pond as soon as possible. The pond has some leaves arranged in a straight line. Each frog has the strength to jump exactly K leaves. For example, a  frog having strength 2 will visit the leaves 2, 4, 6, ...  etc. while crossing the pond.
...
class Solution {
  public:
    int unvisitedLeaves(int n, int leaves, int frogs[]) {
        vector<int> vis_leaves(leaves+1, 0);
        
        //mark visited for all multiple of frogs[i]
        for(int i = 0; i < n; i++) {
            if(leaves < frogs[i]) {
                continue;
            }
            if(!vis_leaves[frogs[i]]) {
        ...
        //now count all unvisited leaves and return
        int ans = 0;
